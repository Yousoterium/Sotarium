...
UI = (function()
	...
	function UI:CreateWindow(cfg)
		...
		-- Logo icon removed. Title now sits at the left padding.
		new("TextLabel", {
		...
		-- ===== Loading Overlay (sized to Main, rounded corners) =====
		...
		local LoadingGroup = new("Frame", {
			Name = "LoadingGroup",
			Size = UDim2.new(0, 140, 0, 150),
			Position = UDim2.new(0.5, -70, 0.5, -75),
			BackgroundTransparency = 1,
			ZIndex = 51,
		...
		task.spawn(function()
			...
			local breatheUp = TweenService:Create(LogoContainer,
				TweenInfo.new(holdDuration * 0.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
				{ Size = UDim2.new(0, 120, 0, 120), Position = UDim2.new(0.5, -60, 0, -5) })
			breatheUp:Play()
